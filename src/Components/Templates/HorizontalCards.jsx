        ...
        {data && data.map((card, index) => (
            ...
            style={{
              background: `linear-gradient(rgba(0,0,0,.2), rgba(0,0,0,.5), rgba(0,0,0,.7)), url(${
                card.backdrop_path || card.poster_path
                  ? `https://image.tmdb.org/t/p/original/${card.backdrop_path || card.poster_path}`
                  : noImg
              })`,
          ...
          >
            <div className="cardDetails flex flex-col p-4">
              ...
              {card?.original_name || card?.title || card?.name || card?.original_title}
              </h1>

              <p className="text-sm">
              {card?.overview ? card.overview.slice(0, 50) : ''}
                <Link
                  to={`/${card.media_type}/details/${card.id}`}
