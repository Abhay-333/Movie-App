      ...
      <div className="cards h-[20rem] w-full gap-5 flex">
        {data.map((card, index) => (
          <Link
            ...
            key={index}
            style={ card.profile_path || card.backdrop_path || card.profile_path || card.poster_path ?{ 
          ...
          >
            <div className="cardDetails flex flex-col p-4">
              ...
              <p className="text-sm">
                ...
                <Link to={`/${card.media_type}/details/${card.id}`} className="text-blue-400 font-semibold ">...more</Link>
              </p>
            </div>
          </Link>
        ))}
